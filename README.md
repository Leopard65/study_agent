...
| 资料库 | `/materials` | 上传 PDF/Word/TXT/MD，分块索引，中文关键词检索，删除时同步清理文件和索引 |
...
当前版本尚未实现显式的文件大小、资料总数或批量上传限制：

- 单次上传：前端和后端均为单文件上传
- 文件大小：代码层没有 `MAX_UPLOAD_SIZE`，实际受内存、磁盘、浏览器和服务器限制影响
- 资料数量：没有总数限制，数据保存在本地 SQLite 和 `backend/uploads/`
- OCR 页数：扫描版 PDF 默认只 OCR 前 `OCR_MAX_PAGES=30` 页

建议本地使用时先控制在：

- 文本型 PDF：50MB 以内
- 扫描版 PDF：20MB 以内，30 页以内更稳
- 资料数量：几十到几百份以内

后续建议补充文件大小限制、资料列表分页、批量上传、后台 OCR 任务队列和上传进度显示。
...
截至当前版本，本地冒烟测试覆盖 health、资料上传/检索/删除、学习计划 CRUD、错题本 CRUD、工作台统计，结果为 `47 passed, 0 failed`。
