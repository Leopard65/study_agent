...
passed = 0
failed = 0
# ── Anti-regression: per-domain check counting ──
_domain_counts: dict[str, int] = {}  # domain_name -> check count
_current_domain: str = ""


def _set_domain(name: str):
    """Mark the start of a test domain so check() can track counts."""
    global _current_domain
    _current_domain = name


def check(name: str, condition: bool, detail: str = ""):
    global passed, failed
    if _current_domain:
        _domain_counts[_current_domain] = _domain_counts.get(_current_domain, 0) + 1
    if condition:
        passed += 1
        print(f"  PASS  {name}")
    else:
...
def test_materials(client: TestClient):
    _set_domain("materials")
    # ── 2. Upload material ──
    ...
    return material_id, stored_filename


def test_plan(client: TestClient):
    _set_domain("plan")
    # ── 5. Study Plan CRUD ──
    print("\n[5] POST /api/plan")
...
def test_exam(client: TestClient):
    _set_domain("exam")
...
def test_review_settings(client: TestClient):
    _set_domain("review_settings")
    # ── 18. Review settings ──
...
def test_sessions(client: TestClient):
    _set_domain("sessions")
    # ── 22. Study sessions ──
...
def test_error_stats(client: TestClient):
    _set_domain("error_stats")
    # ── 23. Error stats ──
...
def test_materials_parse_jobs(client: TestClient):
    _set_domain("materials_parse_jobs")
...
def test_review_queue(client: TestClient):
    _set_domain("review_queue")
    # ── 29. Review queue ──
    ...
    check("FastAPI version=0.7.0", fastapi_app.version == "0.7.0", f"got {fastapi_app.version}")
...
def test_api_key_503(client: TestClient):
    _set_domain("api_key_503")
...
def test_zip(client: TestClient):
    _set_domain("zip")
    ...
    detail = r.json()
    # After overwrite, material is re-queued for parsing; status may be pending/ready/processing
    check("imported material: status pending or ready or processing",
          detail.get("status") in ("pending", "ready", "processing"), f"got {detail.get('status')}")
...
def test_export_import_advanced(client: TestClient):
    _set_domain("export_import_advanced")
    ...
    # ── 70. study_sessions import boundary validation ──
    print("\n[70] study_sessions import boundary validation")
    
    def _make_session_backup(sessions):
        return {
            "exported_at": "2026-01-01T00:00:00Z", "version": "0.3",
            "materials": [], "material_chunks_count": 0,
            "chat_history": [], "error_book": [], "study_plans": [],
    ...
    r = client.post("/api/import/json", json=_make_session_backup([
        {"subject": "边界测试-nostart", "note": "ns", "ended_at": "2026-06-01T22:00:00Z", "duration_minutes": 60}
    ]), params={"strategy": "skip"})
    check("no_started 200", r.status_code == 200)
    check("no_started invalid>=1", r.json().get("sessions_invalid", 0) >= 1, f"got {r.json()}")
    check("no_started imported=0", r.json()["sessions_imported"] == 0)
    
    # 70h: ended_at < started_at → invalid
    r = client.post("/api/import/json", json=_make_session_backup([
        {"subject": "边界测试-reverse", "note": "rv", "started_at": "2026-06-02T10:00:00Z",
         "ended_at": "2026-06-02T09:00:00Z", "duration_minutes": 60}
    ]), params={"strategy": "skip"})
    check("reverse 200", r.status_code == 200)
    check("reverse invalid>=1", r.json().get("sessions_invalid", 0) >= 1, f"got {r.json()}")
    check("reverse imported=0", r.json()["sessions_imported"] == 0)
    
    # 70i: active session with started_at in the future → invalid
    r = client.post("/api/import/json", json=_make_session_backup([
        {"subject": "边界测试-future", "note": "fu", "started_at": "2099-01-01T00:00:00Z",
         "ended_at": None, "duration_minutes": 0}
    ]), params={"strategy": "skip"})
    check("future_active 200", r.status_code == 200)
    check("future_active invalid>=1", r.json().get("sessions_invalid", 0) >= 1, f"got {r.json()}")
    check("future_active imported=0", r.json()["sessions_imported"] == 0)
    
    # 70j: preview sessions_invalid mixed stats
    r = client.post("/api/import/preview", json=_make_session_backup([
        # valid
        {"subject": "边界预览-ok", "note": "ok", "started_at": "2026-06-03T10:00:00Z",
         "ended_at": "2026-06-03T11:00:00Z", "duration_minutes": 60},
        # invalid: missing started_at
        {"subject": "边界预览-bad", "note": "bad"},
        # invalid: ended < started
        {"subject": "边界预览-rev", "note": "rev", "started_at": "2026-06-03T12:00:00Z",
         "ended_at": "2026-06-03T11:00:00Z", "duration_minutes": 60},
    ]), params={"strategy": "skip"})
    check("preview mixed 200", r.status_code == 200)
    pv = r.json()
    check("preview sessions_invalid=2", pv.get("sessions_invalid") == 2, f"got {pv.get('sessions_invalid')}")
    check("preview study_sessions_count=1", pv.get("study_sessions_count") == 1, f"got {pv.get('study_sessions_count')}")
    ...
    async def _cleanup_boundary():
        async with async_session() as session:
            await session.execute(text(
                "DELETE FROM study_sessions WHERE subject LIKE '边界测试-%' OR subject LIKE '边界预览-%'"
            ))
            await session.commit()
    loop_bnd = asyncio.new_event_loop()
    loop_bnd.run_until_complete(_cleanup_boundary())
    loop_bnd.close()


def test_parse_worker_race(client: TestClient):
    _set_domain("parse_worker_race")
...
def test_database_paths(client: TestClient):
    _set_domain("database_paths")
    """Sections 71-72: Database directory auto-creation + path normalization."""
    from app.config import _BACKEND_DIR, _resolve_sqlite_url
    from app.database import _ensure_db_dir
    import tempfile as _tf

    # ── 71. _ensure_db_dir() behaviour ──
    print("\n[71] _ensure_db_dir() behaviour")

    # 71a: non-existent sqlite directory gets created
    _td = _tf.mkdtemp(prefix="smoke_ensure_")
    _test_db = os.path.join(_td, "sub", "deep", "test.db")
    _test_url = f"sqlite+aiosqlite:///{_test_db.replace(chr(92), '/')}"
    _ensure_db_dir(_test_url)
    check("71a: non-existent sqlite dir created", os.path.isdir(os.path.join(_td, "sub", "deep")))
    shutil.rmtree(_td, ignore_errors=True)

    # 71b: existing directory is idempotent
    _td2 = _tf.mkdtemp(prefix="smoke_ensure2_")
    _test_db2 = os.path.join(_td2, "test.db")
    _test_url2 = f"sqlite+aiosqlite:///{_test_db2.replace(chr(92), '/')}"
    _ensure_db_dir(_test_url2)
    _ensure_db_dir(_test_url2)  # second call should not fail
    check("71b: existing dir idempotent", os.path.isdir(_td2))
    shutil.rmtree(_td2, ignore_errors=True)

    # 71c: non-sqlite URL is a no-op (no crash, no dir created)
    _td3 = _tf.mkdtemp(prefix="smoke_ensure3_")
    _no_op_path = os.path.join(_td3, "no_such_dir", "x.db")
    _ensure_db_dir(f"postgresql://localhost/{_no_op_path}")
    check("71c: non-sqlite URL no-op", not os.path.isdir(os.path.join(_td3, "no_such_dir")))
    shutil.rmtree(_td3, ignore_errors=True)

    # 71d: sqlite:///:memory: is a no-op
    _ensure_db_dir("sqlite+aiosqlite:///:memory:")
    check("71d: :memory: no-op (no crash)", True)

    # 71e: sqlite URL with query string still creates directory
    _td4 = _tf.mkdtemp(prefix="smoke_ensure4_")
    _qs_db = os.path.join(_td4, "qs_sub", "qs.db")
    _qs_url = f"sqlite+aiosqlite:///{_qs_db.replace(chr(92), '/')}?mode=ro&cache=shared"
    _ensure_db_dir(_qs_url)
    check("71e: query-string sqlite URL creates dir", os.path.isdir(os.path.join(_td4, "qs_sub")))
    shutil.rmtree(_td4, ignore_errors=True)

    # 71f: empty path sqlite URL does not crash
    _ensure_db_dir("sqlite+aiosqlite:///")
    check("71f: empty path sqlite URL no crash", True)

    # 71g: data dir exists after startup (created by init_db)
    auto_db_dir = os.path.join(str(_BACKEND_DIR), "data")
    check("71g: data dir exists after startup", os.path.isdir(auto_db_dir))

    # 71h: app.db path is absolute
    auto_db_path = os.path.join(auto_db_dir, "app.db")
    check("71h: app.db path is absolute", os.path.isabs(auto_db_path))

    # ── 72. _resolve_sqlite_url() behaviour ──
    print("\n[72] _resolve_sqlite_url() behaviour")
    expected_prefix = str(_BACKEND_DIR).replace("\\", "/")

    # 72a: relative sqlite path resolves to absolute under backend/
    resolved = _resolve_sqlite_url("sqlite+aiosqlite:///./data/app.db")
    check("72a: relative resolved to absolute", resolved != "sqlite+aiosqlite:///./data/app.db")
    check("72b: resolved starts with sqlite", resolved.startswith("sqlite+aiosqlite:///"))
    resolved_path_part = resolved.replace("\\", "/").replace("sqlite+aiosqlite:///", "")
    check("72c: default URL under backend", resolved_path_part.startswith(expected_prefix),
          f"got={resolved[:80]}")

    # 72d: absolute path stays unchanged
    abs_url = "sqlite+aiosqlite:///C:/data/test.db"
    check("72d: absolute path preserved", _resolve_sqlite_url(abs_url) == abs_url)

    # 72e: :memory: stays unchanged
    mem_url = "sqlite+aiosqlite:///:memory:"
    check("72e: :memory: preserved", _resolve_sqlite_url(mem_url) == mem_url)

    # 72f: non-sqlite stays unchanged
    pg_url = "postgresql://user:pass@localhost/mydb"
    check("72f: non-sqlite preserved", _resolve_sqlite_url(pg_url) == pg_url)

    # 72g: query string preserved
    qs_url = "sqlite+aiosqlite:///./data/app.db?mode=ro"
    qs_resolved = _resolve_sqlite_url(qs_url)
    check("72g: query string preserved", "mode=ro" in qs_resolved, f"got={qs_resolved}")

    # 72h: special query chars (space, &, =) preserved
    sp_url = "sqlite+aiosqlite:///./data/app.db?key=val%20ue&foo=bar%26baz"
    sp_resolved = _resolve_sqlite_url(sp_url)
    check("72h: special query chars preserved", "key=" in sp_resolved and "foo=" in sp_resolved,
          f"got={sp_resolved}")

    # 72i: multiple params preserved
    multi_url = "sqlite+aiosqlite:///./data/app.db?a=1&b=2&c=3"
    multi_resolved = _resolve_sqlite_url(multi_url)
    check("72i: multi params preserved", "a=1" in multi_resolved and "b=2" in multi_resolved and "c=3" in multi_resolved,
          f"got={multi_resolved}")

    # 72j: duplicate params NOT folded (?a=1&a=2 kept as-is)
    dup_url = "sqlite+aiosqlite:///./data/app.db?a=1&a=2"
    dup_resolved = _resolve_sqlite_url(dup_url)
    check("72j: duplicate params not folded", "a=1" in dup_resolved and "a=2" in dup_resolved,
          f"got={dup_resolved}")

    # 72k: get_settings().database_url already normalized
    from app.config import get_settings as _gs
    sdb = _gs().database_url
    check("72k: settings.database_url normalized", sdb != "sqlite+aiosqlite:///./data/app.db")
    check("72l: settings.database_url absolute", not sdb.replace("sqlite+aiosqlite:///", "").startswith("./"))

    # 72m: engine.url.database matches settings
    from app.database import engine
    eng_db = str(engine.url.database).replace("\\", "/")
    sdb_path = sdb.replace("sqlite+aiosqlite:///", "").replace("\\", "/")
    check("72m: engine.url.database matches settings",
          eng_db == sdb_path or eng_db.replace("/", "\\") == sdb_path.replace("/", "\\"),
          f"engine={eng_db[:60]}  settings={sdb_path[:60]}")

    # 72n: upload_dir relative path anchored to backend/, absolute stays absolute
    resolved_upload = _gs().upload_dir
    if os.environ.get("UPLOAD_DIR"):
        check("72n: upload_dir uses env override", os.path.isdir(resolved_upload))
    else:
        check("72n: upload_dir resolved under backend",
              resolved_upload.replace("\\", "/").startswith(expected_prefix),
              f"got={resolved_upload[:80]}")
    # Absolute upload_dir stays absolute
    check("72o: upload_dir is absolute", os.path.isabs(resolved_upload),
          f"got={resolved_upload}")


def test_settings_app(client: TestClient):
    _set_domain("settings_app")
    """Section 73: /api/settings/app contract."""
    # ── 73. /api/settings/app contract ──
    print("\n[73] /api/settings/app contract")
    r = client.get("/api/settings/app")
    check("settings/app 200", r.status_code == 200, f"got {r.status_code}")
    sdata = r.json()
    check("settings/app has ai_configured", "ai_configured" in sdata)
    check("settings/app has openai_base_url", "openai_base_url" in sdata)
    check("settings/app has openai_model", "openai_model" in sdata)
    check("settings/app has ocr_enabled", "ocr_enabled" in sdata)
    check("settings/app NO ocr_available", "ocr_available" not in sdata,
          f"got keys={list(sdata.keys())}")

    # 73b: /api/health DOES have ocr_available
    r = client.get("/api/health")
    hdata = r.json()
    check("health has ocr_available", "ocr_available" in hdata)

    # 73c: save settings -> cache cleared -> re-read reflects new value
    from app.config import get_settings as _gs
    original_model = _gs().openai_model
    test_model = "smoke-test-model-73c"
    r = client.put("/api/settings/app", json={"openai_model": test_model})
    check("settings PUT 200", r.status_code == 200, f"got {r.status_code}")
    check("settings PUT ok", r.json().get("ok") is True)
    r2 = client.get("/api/settings/app")
    check("settings GET reflects new model", r2.json().get("openai_model") == test_model,
          f"got {r2.json().get('openai_model')}")
    check("PUT response has confirmed model", r.json().get("openai_model") == test_model,
          f"got {r.json().get('openai_model')}")
    client.put("/api/settings/app", json={"openai_model": original_model})

    # 73d: clear API key
    r = client.put("/api/settings/app", json={"clear_api_key": True})
    check("clear key 200", r.status_code == 200, f"got {r.status_code}")
    check("clear key ok", r.json().get("ok") is True)
    check("clear key ai_configured=false", r.json().get("ai_configured") is False,
          f"got {r.json().get('ai_configured')}")
    check("clear key response has confirmed values", r.json().get("openai_model") is not None)

    # 73e: empty model rejected
    r = client.put("/api/settings/app", json={"openai_model": ""})
    check("empty model 422", r.status_code == 422, f"got {r.status_code}")

    # 73f: empty base_url rejected
    r = client.put("/api/settings/app", json={"openai_base_url": ""})
    check("empty base_url 422", r.status_code == 422, f"got {r.status_code}")

    # 73g: no fields rejected
    r = client.put("/api/settings/app", json={})
    check("no fields 422", r.status_code == 422, f"got {r.status_code}")

    # 73h: clear_api_key + openai_api_key conflict -> 422
    r = client.put("/api/settings/app", json={"clear_api_key": True, "openai_api_key": "sk-test"})
    check("conflict clear+set 422", r.status_code == 422, f"got {r.status_code}")
    check("conflict mentions both", "不能同时" in r.json().get("detail", ""),
          f"detail={r.json().get('detail')}")

    # 73i: atomic write -- .env file is not corrupted after write
    from app.services.env_store import read_env_lines, _ENV_FILE
    r = client.put("/api/settings/app", json={"openai_model": "atomic-test"})
    check("atomic write 200", r.status_code == 200, f"got {r.status_code}")
    raw_lines = read_env_lines()
    has_model_line = any("OPENAI_MODEL=atomic-test" in line for line in raw_lines)
    check("atomic .env has new model", has_model_line, f"lines={[l for l in raw_lines if 'MODEL' in l]}")
    has_base_url = any("OPENAI_BASE_URL=" in line for line in raw_lines)
    check("atomic .env preserved other keys", has_base_url)
    client.put("/api/settings/app", json={"openai_model": _gs().openai_model})

    # 73j: after clear, same-process read returns ai_configured=false
    client.put("/api/settings/app", json={"clear_api_key": True})
    from app.config import is_ai_configured as _iac
    check("after clear ai_configured=false", _iac() is False, f"got {_iac()}")
...
def run(client: TestClient):
    ...
    test_health(client)
    material_id, stored_filename = test_materials(client)
    test_plan(client)
    test_errors(client)
    test_dashboard(client)
    test_input_validation(client)
    test_exam(client)
    test_export_json(client)
    test_review_settings(client)
    test_import(client)
    test_dashboard_trends(client)
    test_search(client)
    test_sessions(client)
    test_error_stats(client)
    test_materials_parse_jobs(client)
    test_review_queue(client)
    test_api_key_503(client)
    test_zip(client)
    test_maintenance(client)
    test_export_import_advanced(client)
    test_parse_worker_race(client)
    test_database_paths(client)
    test_settings_app(client)
